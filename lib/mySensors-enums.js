...
buildLists('C_', mys.commands, mys.n_commands);
buildLists('S_', mys.sensors, mys.n_sensors);
buildLists('V_', mys.data, mys.n_data);
buildLists('I_', mys.internal, mys.n_internal);
buildLists('ST_', mys.streams, mys.n_streams);
...
exports.parse = function (data, preparePayload) {
    ...
    for (var i=0; i<lines.length; ++i) {
        ...
        try {
            result.type = mys.commands[arr[2]];
            switch (result.num.type) {
                case C_PRESENTATION:
                    result.subType = mys.sensors[result.num.subType];
                    //?? try without subType string ??
                    break;
                case C_SET:
                case C_REQ:
                    result.subType = mys.data[result.num.subType];
                    break;
                case C_INTERNAL:
                    result.subType = mys.internal[result.num.subType];
                    break;
                case C_STREAM:
                    result.subType = mys.streams[result.num.subType];
                    break;
            }
        }
        catch (e) {
